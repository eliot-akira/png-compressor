...
import { ColorType, FilterType, InterlaceMethod } from '../../shared/types.js'
...
function writeUncompressedData(
  ...
  for (; y < image.height; y++) {
    // Filter type
    const filterType = filterPattern
      ? filterPattern[y % filterPattern.length]
      : pickFilterType(ctx.colorType, image, y * image.width * 4, filterFns)
