...
import type { IEncodedPng } from '../api.d.ts'
...
import { ColorType, InterlaceMethod, KnownChunkTypes } from '../shared/types.js'
import type {
  IEncodeContext,
  IEncodePngOptions,
  IImage32,
...
export { EncodeError, EncodeWarning }
...
function getChunkDecoder(type: KnownChunkTypes): Promise<{
  ...
  switch (type) {
    case KnownChunkTypes.tRNS:
      return import(`./chunks/tRNS_encode.js`)
...
export async function encodePng(
  ...
  if (image.data.length !== image.width * image.height * 4) {
    throw new EncodeError(
      `Provided image data length (${
        image.data.length
      }) is not expected length (${image.width * image.height * 4})`,
  ...
  if (ctx.colorType === ColorType.Indexed) {
    const result = (await import(`./chunks/PLTE_encode.js`)).encodeChunk(
      ctx,
      image,
    )
  ...
  if (ctx.useTransparencyChunk) {
    sections.push(
      (await getChunkDecoder(KnownChunkTypes.tRNS)).encodeChunk(ctx, image),
    )
  }
...
function analyze(
  ...
  return {
    ...
    firstTransparentColor:
      transparentColorSet.size > 0
        ? transparentColorSet.values().next().value
        : undefined,
