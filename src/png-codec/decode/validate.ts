...
export function isValidColorType(
  colorType: number,
  bitDepth: number,
): colorType is ColorType {
  return (
    ...
    (colorType === ColorType.GrayscaleAndAlpha &&
      bitDepth >= 8 &&
      bitDepth <= 16) ||
...
export function isValidFilterMethod(
  filterMethod: number,
): filterMethod is FilterMethod {
  return filterMethod === FilterMethod.Adaptive
}

export function isValidInterlaceMethod(
  interlaceMethod: number,
): interlaceMethod is InterlaceMethod {
  return (
