...
export function parseChunk(
  ...
  const trnsChunk = ctx.metadata.find((e) => e.type === 'tRNS') as
    | IPngMetadataTransparency
    | undefined
    ...
    switch (filterType) {
      ...
      case FilterType.Average: {
        ...
        for (; x < bpp; x++) {
          bi = i - width * bpp
          result[i] =
            decompressed[lineOffset + x] +
            Math.floor(0 + (bi < 0 ? 0 : result[bi])) / 2
          i++
        }
        break
      }
      ...
      case FilterType.Paeth: {
        for (; x < bpp; x++) {
          ...
          result[i] =
            (decompressed[lineOffset + x] +
              paethPredicator(0, bi < 0 ? 0 : result[bi], 0)) %
            256
          i++
        }
        break
...
type DefilterFunction = (
  filt: Uint8Array | Uint16Array,
  filtX: number,
  recon: Uint8Array | Uint16Array,
  reconX: number,
) => number

function buildDefilterFunction(
  bpp: number,
  bpl: number,
  width: number,
  ...
  switch (filterType) {
    ...
    case FilterType.Sub:
      return (filt, filtX, recon, reconX) =>
        (filt[filtX] + recon[Math.floor(reconX - bpp)]) % 256
    case FilterType.Up:
      return (filt, filtX, recon, reconX) => {
    ...
    case FilterType.Average:
      return (filt, filtX, recon, reconX) => {
        ...
        return (
          filt[filtX] +
          Math.floor((ai < 0 ? 0 : recon[ai]) + (bi < 0 ? 0 : recon[bi])) / 2
        )
      }
    case FilterType.Paeth:
      return (filt, filtX, recon, reconX) => {
...
function deinterlaceAdam7(
  ...
  for (let pass = 0; pass < 7; pass++) {
      ...
      for (let x = 0; x < passWidth; x++) {
        if (header.bitDepth < 8) {
          ...
          const resultPosition =
            (pixelsPerByte - 1 - (i % 1) * pixelsPerByte) * header.bitDepth
    ...
    dataPointer +=
      passHeight * (1 /*filter type*/ + getBytesPerLine(header, passWidth))
  }
  return result
}

function mapPackedDataToRgba(
  ctx: IDecodeContext,
  header: IPngHeaderDetails,
  packed: Uint8Array,
  palette: IPngPaletteInternal | undefined,
  trnsChunk: IPngMetadataTransparency | undefined,
) {
        ...
        case 4: {
          ...
          for (let y = 0; y < header.height; y++) {
            for (let x = 0; x < header.width; x++) {
              ...
              // Shift bits being considered to RHS
              result[i] >>=
                (pixelsPerByte - (x % pixelsPerByte) - 1) * header.bitDepth
              // Mask out irrelevant bits
              result[i] &= maxValue
          ...
          for (let y = 0; y < header.height; y++) {
            for (let x = 0; x < header.width; x++) {
              ...
              if (header.bitDepth === 16) {
                result[i] =
                  (packed[(y * header.width + x) * bpp] << 8) |
                  packed[(y * header.width + x) * bpp + 1]
              } else {
    ...
    case ColorType.GrayscaleAndAlpha: {
      ...
      for (let y = 0; y < header.height; y++) {
        for (let x = 0; x < header.width; x++) {
          ...
          if (header.bitDepth === 16) {
            result[i + 3] =
              (packed[(y * header.width + x) * bpp + 2] << 8) |
              packed[(y * header.width + x) * bpp + 3]
          } else {
        ...
        case 4: {
          ...
          for (let y = 0; y < header.height; y++) {
            for (let x = 0; x < header.width; x++) {
              i = (y * header.width + x) * 4
              let colorId =
                packed[y * bplCeiled + Math.floor(x * bytesPerPixel)]
              // Shift bits being considered to RHS
              colorId >>=
                (pixelsPerByte - (x % pixelsPerByte) - 1) * header.bitDepth
              // Mask out irrelevant bits
              ...
              if (
                trnsChunk &&
                (trnsChunk.transparency as number[]).length > colorId
              ) {
        ...
        for (let x = 0; x < header.width; x++) {
          ...
          if (header.bitDepth === 16) {
            ...
            result[i + 1] =
              ...
              packed[(y * header.width + x) * bpp + 3]
            result[i + 2] =
              (packed[(y * header.width + x) * bpp + 4] << 8) |
              packed[(y * header.width + x) * bpp + 5]
          } else {
            result[i] = packed[(y * header.width + x) * bpp]
        ...
        for (let x = 0; x < header.width; x++) {
          ...
          if (header.bitDepth === 16) {
            ...
            result[i + 2] =
              ...
              packed[(y * header.width + x) * bpp + 5]
            result[i + 3] =
              (packed[(y * header.width + x) * bpp + 6] << 8) |
              packed[(y * header.width + x) * bpp + 7]
          } else {
            result[i] = packed[(y * header.width + x) * bpp]
            result[i + 1] = packed[(y * header.width + x) * bpp + 1]
            result[i + 2] = packed[(y * header.width + x) * bpp + 2]
...
function applyTransparency(
  ...
  trnsChunk: IPngMetadataTransparency,
) {
  ...
  if (header.colorType === ColorType.Grayscale) {
    const shade =
      (maxDataValue / maxEncodedValue) * (trnsChunk.transparency as number)
  ...
  if (header.colorType === ColorType.Truecolor) {
    const channels = [
      (maxDataValue / maxEncodedValue) *
        (trnsChunk.transparency as [number, number, number])[0],
      (maxDataValue / maxEncodedValue) *
        (trnsChunk.transparency as [number, number, number])[1],
      (maxDataValue / maxEncodedValue) *
        (trnsChunk.transparency as [number, number, number])[2],
    ]
    for (let i = 0; i < data.length; i += 4) {
      if (
        data[i] === channels[0] &&
